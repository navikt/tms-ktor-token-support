...
internal fun AuthenticationConfig.registerAzureProviderMock(authenticatorName: String?, authInfo: AuthInfo) {
...
private class AccessTokenAuthenticationProvider(
    ...
    override suspend fun onAuthenticate(context: AuthenticationContext) {
        if (authInfo.alwaysAuthenticated) {
            log.debug {"Auth is valid as entra-id-mock is set to never authorized." }
            context.principal(AzurePrincipalBuilder.createPrincipal(authInfo))
        } else {
            log.debug {"Responding 401 as entra-id-mock is set to never authorized." }
