...
internal class AzureAuthIT {
    ...
    @Test
    fun `Should respond ok if valid bearer token is provided in entra-id-auth header`() = testApplication {
    ...
    @Test
    fun `Should prioritize well-formed entra-id-auth header over regular auth header`() = testApplication {
    ...
    @Test
    fun `Should fall back to regular auth header if entra-id-auth header is malformed`() = testApplication {
